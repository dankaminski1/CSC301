...
public class RSequentialSearchST<Key extends Comparable<Key>, Value> {
    ...
    private Value get(Node first, Key key) {
    	if(first == null) 
    		return null;
    ...
    private Node put(Node first, Key key, Value val){
     if(first == null) {
    	 Node newFirst = new Node(key, val, null);
    	 n++;
    	 return newFirst;
     }
     if (key.compareTo(first.key) < 0) {
     ...
     first.next = put(first.next, key, val);
     return first;
    }
